...
class RSAEncryptor:
    def __init__(self):
        self.e = 65537
        self.generate_modulus()
        self.encrypt_flag()

    def generate_modulus(self):
        self.g1 = getPrime(512)
        self.g2 = getPrime(512)
        self.g3 = getPrime(512)

        while True:
            self.a = random.randint(2**511, 2**512)
            self.b = random.randint(2**511, 2**512)
            self.c = random.randint(2**511, 2**512)
            if gmpy2.is_prime(2 * self.g1 * self.a + 1) and gmpy2.is_prime(2 * self.g2 * self.b + 1) and gmpy2.is_prime(2 * self.g3 * self.c + 1):
                break

        self.h = 2 * self.g1 * self.a * self.b + self.b * self.c + 3 * self.g2 * self.a + self.g3 * self.c
        self.N = self.h * self.g1 * self.g2 * self.g3 + 1
    ...
    def encrypt_flag(self):
        flag = "QHCTF{xxxxxxx}"
        self.flag = bytes_to_long(flag.encode())
        self.enc = self.encrypt(self.flag)
        self.show_info()
        print(f'enc={self.enc}')
...
'''
N=65537
enc=
'''
```

#### 解题思路

+ $h=2ab*g1+bc+3a*g2+c*g3$

+ $N=h*g1*g2*g3+1$

   $=(2ab*g1^2*g2*g3+3a*g1*g2^2*g3)+bc*g1*g2*g3+c(g1*g2*g3^2)+1$

   $=(g1*g2*g3)(2ab*g1+3a*g3+bc+c*g3)+1$
